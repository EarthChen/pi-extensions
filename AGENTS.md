...
- 包管理器: pnpm + workspace
- 分发: 每扩展独立 npm 包(`@earthchen/pi-<name>`)/ git(`pi install`)
...
- 每个扩展 = `packages/<name>/`:独立 `package.json`(name `@earthchen/pi-<name>`)、`<name>.ts`(默认导出 `ExtensionAPI` 注册函数)、`README.md`、`LICENSE`。
- 每包的 `pi.extensions: ["./<name>.ts"]` 是该扩展的安装入口;装 `npm:@earthchen/pi-<name>` 只装这一个。
...
- 包名 `@earthchen/pi-<name>`;版本由根同步,**不要**手动改各包 `version`。
- `typebox` 版本锁死(当前 `1.3.8`,取自用环境);发布前确认公网可解析。

## Rules

- 加扩展必须新建 `packages/<name>/` 并跑 `node scripts/sync-version.mjs`,保证版本与根一致;漏同步会导致发版版本错乱。
